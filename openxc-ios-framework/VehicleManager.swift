...
open class VehicleManager: NSObject {
    ...
    open func sendCommand<T: AnyObject>(_ cmd:VehicleCommandRequest, target: T, action: @escaping (T) -> (NSDictionary) -> ()) -> String {
        ...
        // if we have a trace input file, ignore this request! Change by ranjan
        if (TraceFileData._sharedInstance.traceFilesourceEnabled) {return ""}
    ...
    // common function for sending a VehicleCommandRequest Protobuf mode
...
//        }
    ...
    fileprivate func jsonDecoding(data_chunk:NSMutableData){
        ...
        if TraceFileData.sharedInstance.traceFilesourceTimeTracking {
            let msTimeNow = Int(Date.timeIntervalSinceReferenceDate*1000)
            if TraceFileData.sharedInstance.traceFilesourceLastMsgTime == 0 {
                // first time
                TraceFileData.sharedInstance.traceFilesourceLastMsgTime = timestamp
                TraceFileData.sharedInstance.traceFilesourceLastActualTime = msTimeNow
            }
            let msgDelta = timestamp - TraceFileData.sharedInstance.traceFilesourceLastMsgTime
            let actualDelta = msTimeNow - TraceFileData.sharedInstance.traceFilesourceLastActualTime
        ...
        if let event = json["event"] as? NSString {
            ...
            self.eventedMeasurementrsp(json:json as [String:AnyObject],event:event,timestamp:timestamp)
        }
        ...
        // and send it to the trace file writer Change By ranjan
        if TraceFileData.sharedInstance.traceFilesinkEnabled {
            let str = String(data: data_chunk as Data,encoding: String.Encoding.utf8)
            TraceFileData.sharedInstance.traceFileWriter(str!)
        }
