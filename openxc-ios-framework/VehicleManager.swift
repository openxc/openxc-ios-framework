...
open class VehicleManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    //Chester modified -- disconnect the VI currently connected.
    open func disconnect() {
        if connectionState != .connected && connectionState != .operational {
            vmlog("VehiclManager has not connection to VI")
            return
        }
        vmlog("VehicleManager disconnect started")
    ...
    fileprivate func sendDiagCommon(_ cmd:VehicleDiagnosticRequest) {
        ...
        if cmd.payload != "" {
            //changed May.3 fixed the escape character bug which causing DTC not been processed in firmware
//            cmdjson.append(",\"payload\": 0x8f")
            cmdjson.append(",\"payload\":")
            let char = "\""
            cmdjson.append(char)
            cmdjson.append(String(cmd.payload))
            cmdjson.append(char)
        }

        
//        cmdjson.append("{\"action\":\"add\",\"request\":{\"bus\": \(cmd.bus),\"id\":\(cmd.message_id),\"mode\":\(cmd.mode)")
//        
//        if cmd.pid != nil {
//            cmdjson.append(",\"pid\":\(cmd.pid!)")
//        }
...
//        print("payload in command, framework : \(cmd.payload)")
        
//        cmdjson.append("},\"command\":\"diagnostic_request\"}\0")
...
//            let test = sendBytes as NSData
    ...
    open func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if error != nil {
            //Unexpected disconnection will print error here.
            vmlog(error)
            let vc = UIApplication.topViewController()
            let alert = UIAlertController(title: "DET-Lite", message: (error?.localizedDescription)! + "\n Please restart DET-Lite and the Vehicle Interface.", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "Okay", style: UIAlertActionStyle.default, handler: nil))
            vc?.present(alert, animated: true, completion: nil)
        }
        
        //Chester modified --- disconnect the peripheral now
